...
This usually leads to another query - Who will be responsible for managing the external node ? loxilb docker is a self-contained entity and easily managed with simple tools like docker, containerd, podman etc. It can be independently restarted/upgraded anytime and kube-loxilb will make sure all the k8s service LBs are properly configured each time. 
